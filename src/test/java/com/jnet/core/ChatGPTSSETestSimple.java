...
/**
 * ChatGPT SSE 流式测试 - 简化版本
 * 直接运行此类测试真实的 ChatGPT API
 *
 * 用法:
 *   java com.jnet.core.ChatGPTSSETestSimple
 *
 * 配置:
 ...
 * @version 3.1
 */
public class ChatGPTSSETestSimple {
    ...
    public static void main(String[] args) {
        System.out.println("=== ChatGPT SSE 流式测试 - 简化版 ===\n");

        // 检查 API key
        if (API_KEY.equals("YOUR_API_KEY_HERE") || API_KEY.isEmpty()) {
            System.out.println("❌ 请先配置 API_KEY！");
            System.out.println("   编辑 " + ChatGPTSSETestSimple.class.getName() + " 类");
            System.out.println("   将 YOUR_API_KEY_HERE 替换为真实的 API Key\n");
            return;
        }
    ...
    private static void sendSSERequest(String userMessage) throws Exception {
        ...
        System.out.println("📤 使用 curl 发送请求...");
     ...
     * 从 JSON 中提取 usage 字段
     */
    private static String extractUsageFromJson(String json) {
        try {
            if (json.contains("\"usage\":")) {
                int usageStart = json.indexOf("\"usage\":{");
                int usageEnd = json.indexOf("}", usageStart) + 1;
                String usage = json.substring(usageStart + 9, usageEnd);

                // 简单格式化
                usage = usage.replace("\"", "")
                        .replace("prompt_tokens", "提示词")
                        .replace("completion_tokens", "回答")
                        .replace("total_tokens", "总计")
                        .replace(",", ", ");
